function paramstruct = initializeparams_v3(numfilters,numsamples,param_approx)
...
for i = 1 : numfilters
    ...
    % initial values
    paramstruct.G{i}    = param_approx(i,2);
    paramstruct.fc{i}   = param_approx(i,1);
    paramstruct.fb{i}   = param_approx(i,1) / param_approx(i,3);
    paramstruct.y{i}    = zeros(1,numsamples);
    
    if i == 1
        paramstruct.filtertype{i}   = 'lowshelving'; 
    elseif i == numfilters
        paramstruct.filtertype{i}   = 'highshelving';
    else
        paramstruct.filtertype{i}   = 'peak';
    end
